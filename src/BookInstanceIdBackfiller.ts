...
export class BookInstanceIdBackfiller {
    ...
    public async backfill(): Promise<void> {
        ...
        const updateQueries = this.generateUpdateSqlQuery(
        ...
        if (updateQueries && updateQueries.length) {
            ...
            this.performUpdate("bloomreadertest.pages_read", updateQueries);
    ...
    private generateUpdateSqlQuery(
    ...
    private async performUpdate(tableName: string, updateQueries: string[]) {
        const queryPart1 = `SELECT COUNT(DISTINCT title) AS countDistinctProblemTitles FROM ${tableName} WHERE book_instance_id IS NULL; `;
        ...
        const beforeCount = results[0].rows[0].countDistinctProblemTitles;
        const afterCount = results[2].rows[0].countDistinctProblemTitles;
